...
 
    for(int i =n-1 ;i>=0 ;i--){
        for(int j = 0; j<k+1 ;j++){
            ...
            for(int t = i+1 ; t<=n  ; t++){
                int removed_cnt = t-i-1;
                if(j>=removed_cnt){
                    int time_to_t = (coordinate[t]-coordinate[i])*time[i];
                    int time_from_t_to_end = dp[t][j-removed_cnt];
 
                    if(time_from_t_to_end!=INT_MAX){
                        dp[i][j] = min(dp[i][j],time_from_t_to_end+time_to_t);
                    }
                }
            }
 
