...
vector<vector<vector<int>>> memo; // 3D DP table
 
int dp(int i, int k, int sweet_eaten_till_now) {
    if (k == 0) return 0;
    if (i == 0&&k!=0) return INT_MAX;
 
    // Check memo table
    int &res = memo[i][k][sweet_eaten_till_now];
    if (res != -1) return res;
 
    // Compute current day (1-based)
    int day = (sweet_eaten_till_now + 1 + m - 1) / m;
 
    // Take this sweet
    int take = INT_MAX;
    int sub = dp(i - 1, k - 1, sweet_eaten_till_now + 1);
    if (sub != INT_MAX)
        take = day * sweetness[i] + sub;
 
    // Don't take this sweet
    int not_take = dp(i - 1, k, sweet_eaten_till_now);
 
...
 
    sort(all(sweetness)); // You already do this correctly
 
