...
// -------------------- Solve Function --------------------
 
int max_subsequence(vector<int>&arr, int i,int prev){
    if(i>=arr.size()){
        return 0;
    }
    ...
    if(prev==-1||prev==arr[i]-1){
        take =max(take,1+ max_subsequence(arr,i+1,arr[i]));
    }
    int not_take = max(take,max_subsequence(arr,i+1,prev));
 
...
void solve() {
    ...
    vector<int>next(n,-1);
 
    map<int,int>dp,lastIndex;
 
    for(int i = n-1;i>=0 ;i--){
        dp[arr[i]] = dp[arr[i]+1]+1;
        if(lastIndex.count(arr[i]+1)){
            next[i] = lastIndex[arr[i]+1];
        }
        lastIndex[arr[i]] = i;
        if(dp[arr[i]]>ans){
            ans = dp[arr[i]];
