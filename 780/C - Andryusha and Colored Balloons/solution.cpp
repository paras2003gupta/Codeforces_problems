...
// -------------------- Solve Function --------------------
void dfs(vector<int>&parent , vector<int>adj[], int node,int par){
    parent[node] = par;
 
    for(auto child:adj[node]){
        if(child==par)continue;
 
        dfs(parent,adj,child,node);
    }
 
}
void solve() {
    ...
    dfs(parent,adj,1,0);
 
    //this will fill our parent array
    colour[1] = 1;
    int mx = 1;
    queue<int>q;
    // int cnt = 2;
...
 
        ...
        for(auto it:adj[front]){
            if(it!=parent[front]){
                while(cnt==colour[front]||cnt==colour[parent[front]])cnt++;
                colour[it] = cnt;
                mx = max(mx,cnt);
                cnt++;
                q.push(it);
