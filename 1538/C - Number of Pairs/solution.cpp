...
int get_pairs_whose_sum_less_thn_x(vector<int>&arr,int mark){
        int i = 0;
        int n = arr.size();
        int j = n-1;
        int ans = 0;
...
void solve() {
    ...
    sort(all(arr));
 
    int ans1 = get_pairs_whose_sum_less_thn_x(arr,r);
    int ans2 = get_pairs_whose_sum_less_thn_x(arr,l-1);
    cout<<ans1-ans2<<endl;
    return ;
 
}
